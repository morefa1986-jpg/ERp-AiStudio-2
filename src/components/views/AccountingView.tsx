...
export const AccountingView: React.FC = () => {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    if (currencies.size > 1) { setError('سند دستی چندارزی مجاز نیست؛ ابتدا سند تبدیل ارز ثبت کنید.'); return; }
  ...
  return <div className="space-y-6 pb-12 animate-fadeIn">
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div><h1 className="text-xl font-black text-white flex items-center gap-2"><Calculator className="w-6 h-6 text-amber-400" />حسابداری دوبل و دفتر کل</h1><p className="text-xs text-slate-400 mt-1">ارجاع خطوط سند اکنون با Account.id واقعی انجام می‌شود؛ کد حساب فقط برای نمایش است.</p></div>

...
export const BiometricsView: React.FC = () => {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    if (!selectedPond || validSamples.length < 1 || !operator.trim()) return;
    recordBiometry({
      ...
      notes: notes.trim() || `نمونه‌گیری واقعی شامل ${validSamples.length} قطعه؛ طول فقط در صورت اندازه‌گیری مستقیم ثبت شده است.`,
    });
  ...
  return (
    <div className="space-y-6 pb-12 animate-fadeIn">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div><h1 className="text-xl font-black text-white flex items-center gap-2"><Scale className="w-6 h-6 text-amber-400" />بیومتری واقعی چندنمونه‌ای</h1><p className="text-xs text-slate-400 mt-1">هر وزن یک نمونه مستقل است؛ هیچ طولی از روی وزن تخمین زده نمی‌شود.</p></div>
      ...
      <div className="space-y-3">
        {biometricSessions.map((session) => {
          ...
          return <div key={session.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
            <div className="flex flex-wrap items-center justify-between gap-2 mb-3"><div><strong className="text-white">{session.pondName}</strong><span className="text-xs text-slate-500 mr-2">{session.date}</span></div><span className="text-xs text-slate-400">{session.sampleCount} نمونه · {session.operatorName}</span></div>
      ...
      {showForm && <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4"><div className="bg-slate-900 border border-amber-500/30 rounded-2xl p-6 w-full max-w-3xl max-h-[90vh] overflow-auto">
        ...
        <form onSubmit={submit} className="space-y-4 text-xs">
          <div className="grid md:grid-cols-2 gap-3"><div><label className="text-slate-400 block mb-1">استخر</label><select value={selectedPondId} onChange={(event) => setSelectedPondId(event.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-xl p-2.5 text-white">{ponds.map((pond) => <option key={pond.id} value={pond.id}>{pond.number} — {pond.name}</option>)}</select></div><div><label className="text-slate-400 block mb-1">کارشناس</label><input value={operator} onChange={(event) => setOperator(event.target.value)} required className="w-full bg-slate-800 border border-slate-700 rounded-xl p-2.5 text-white" /></div></div>

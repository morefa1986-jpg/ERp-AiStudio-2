...
type ReportType = 'ponds' | 'feeding' | 'water' | 'mortality' | 'treatments' | 'transfers' | 'inventory' | 'laboratory' | 'processing' | 'sales' | 'receivables' | 'crm' | 'documents' | 'gatehouse' | 'chat' | 'accounting' | 'payroll' | 'audit';
...
const REPORT_LABELS: Record<ReportType, string> = {
  ...
  inventory: 'انبار', laboratory: 'آزمایشگاه', processing: 'فرآوری', sales: 'فروش', receivables: 'مطالبات و سررسید', crm: 'CRM 360 و پیگیری مشتری', documents: 'دبیرخانه و اسناد', gatehouse: 'نگهبانی و برگه خروج', chat: 'چت داخلی', accounting: 'حسابداری', payroll: 'حقوق', audit: 'Audit Trail',
};

const SCOPED_REPORTS = new Set<ReportType>(['ponds', 'feeding', 'water', 'mortality', 'treatments', 'transfers', 'processing', 'sales', 'receivables']);
const DATED_REPORTS = new Set<ReportType>(['feeding', 'water', 'mortality', 'treatments', 'transfers', 'laboratory', 'processing', 'sales', 'receivables', 'crm', 'documents', 'gatehouse', 'chat', 'accounting', 'payroll', 'audit']);
...
const ReportsTablePanel: React.FC = () => {
  ...
  const rows = useMemo<ReportRow[]>((() => {
    switch (reportType) {
      ...
      case 'mortality': return farm.mortalityRecords.filter((row) => scopedPondIds.has(row.pondId) && inDateRange(row.timestamp)).map((row) => ({ timestamp: row.timestamp, pond: row.pondName, species: row.speciesName, sex: row.stockSex || 'Unknown', count: row.count, estimatedWeightKg: row.estimatedWeightKg, chips: row.chipNumbers?.join(' | ') || '', reason: row.reason, photoCount: row.photos?.length || (row.photoUrl ? 1 : 0), photos: row.photos?.map((file) => file.fileName).join(' | ') || '', recordedBy: row.recordedBy }));
      ...
      case 'laboratory': return farm.labSamples.filter((row) => inDateRange(row.collectionDate)).map((row) => ({ sampleCode: row.sampleCode, collectionDate: row.collectionDate, source: row.sourceName, testType: row.testType, status: row.status, criticalCount: row.parametersTested?.filter((item) => item.status === 'Critical').length || 0, attachmentCount: row.attachments?.length || 0, attachments: row.attachments?.map((file) => file.fileName).join(' | ') || '', collector: row.collectorName }));
      ...
      case 'crm': return [
        ...farm.crmActivities.filter((row) => inDateRange(row.createdAt)).map((row) => ({ kind: 'activity', timestamp: row.createdAt, customer: row.customerName, type: row.type, subject: row.subject, outcome: row.outcome, followUpAt: row.followUpAt || '', assignedTo: row.assignedTo || '', relatedProformaId: row.relatedProformaId || '', attachmentCount: row.attachments.length, createdBy: row.createdBy })),
        ...farm.crmReminders.filter((row) => inDateRange(row.dueAt)).map((row) => ({ kind: 'reminder', timestamp: row.dueAt, customer: row.customerName, type: 'Reminder', subject: row.title, outcome: row.status, followUpAt: row.dueAt, assignedTo: row.assignedTo || '', relatedProformaId: '', attachmentCount: 0, createdBy: row.createdBy })),
      ];
      ...
      case 'gatehouse': return farm.gatePasses.filter((row) => inDateRange(row.registeredAt)).map((row) => ({ passNumber: row.passNumber, registeredAt: row.registeredAt, direction: row.direction, status: row.status, vehiclePlateNumber: row.vehiclePlateNumber, carrierVehicleNumber: row.carrierVehicleNumber, vehicleType: row.vehicleType, driverName: row.driverName, driverNationalId: row.driverNationalId, driverPhone: row.driverPhone, cargoOwnerName: row.cargoOwnerName, cargoOwnerNationalId: row.cargoOwnerNationalId, cargoOwnerPhone: row.cargoOwnerPhone, cargoType: row.cargoType, cargoVolume: row.cargoVolume, cargoQuality: row.cargoQuality, waybillNumber: row.waybillNumber, dispatchOrderNumber: row.dispatchOrderNumber, transportPermitNumber: row.transportPermitNumber, originAddress: row.originAddress, destinationAddress: row.destinationAddress, registeredBy: row.registeredBy, approvedBy: row.approvedBy || '', exitSheetIssuedAt: row.exitSheetIssuedAt || '', exitSheetIssuedBy: row.exitSheetIssuedBy || '', exitedAt: row.exitedAt || '' }));
      case 'chat': return farm.chatMessages.filter((row) => inDateRange(row.createdAt)).map((row) => ({ timestamp: row.createdAt, thread: farm.chatThreads.find((thread) => thread.id === row.threadId)?.title || row.threadId, sender: row.senderName, text: row.text.slice(0, 160), attachmentCount: row.attachments.length, attachments: row.attachments.map((file) => file.fileName).join(' | ') }));

...
const REPORT_LABELS: Record<ReportType, string> = {
  ponds: 'وضعیت استخرها', feeding: 'تغذیه', water: 'کیفیت آب', mortality: 'تلفات', treatments: 'درمان‌ها', transfers: 'انتقالات',
...
export const ReportsView: React.FC = () => {
  ...
  }, [farm.ponds, scopeMode, pondId, hallId, hallIds]);
  ...
  const rows = useMemo<ReportRow[]>(() => {
    switch (reportType) {
      case 'ponds': return farm.ponds.filter((pond) => scopedPondIds.has(pond.id)).map((pond) => ({
        pond: pond.name, number: pond.number, hall: farm.halls.find((hall) => hall.id === pond.hallId)?.name || pond.hallId,
        fishCount: pond.fishCount, biomassKg: pond.biomassKg, averageWeightKg: pond.averageWeightKg, fcr: pond.fcr,
        feedingStatus: pond.feedingStatus, sensorQuality: pond.sensorQuality || '', telemetryAt: pond.lastTelemetryTimestamp || '',
      }));
      ...
      case 'mortality': return farm.mortalityRecords.filter((row) => scopedPondIds.has(row.pondId) && inDateRange(row.timestamp)).map((row) => ({ timestamp: row.timestamp, pond: row.pondName, count: row.count, estimatedWeightKg: row.estimatedWeightKg, reason: row.reason, description: row.description, recordedBy: row.recordedBy }));
      ...
      case 'sales': return farm.proformas.filter((row) => inDateRange(row.date)).map((row) => ({ invoice: row.invoiceNumber, date: row.date, customer: row.customerName, country: row.customerCountry, currency: row.currency, total: row.grandTotal, stage: row.stage, fulfilledAt: row.fulfilledAt || '' }));
      ...
      case 'payroll': return farm.payrolls.map((row) => ({ month: row.payrollMonth, employee: row.employeeName, department: row.department, gross: row.grossSalary, deductions: row.socialSecurityInsurance + row.incomeTax + row.loanDeduction, net: row.netPay, currency: row.currency, status: row.paymentStatus }));
  ...
  }, [reportType, farm, scopedPondIds, fromDate, toDate]);
  ...
  return <div className="space-y-5 pb-12 animate-fadeIn">
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col lg:flex-row lg:items-center justify-between gap-4"><div><h1 className="text-xl font-black text-white flex items-center gap-2"><FileText className="w-6 h-6 text-amber-400" />مرکز گزارش‌های عملیاتی</h1><p className="text-xs text-slate-400 mt-1">گزارش بر اساس Scope واقعی و بازه زمانی تولید می‌شود؛ CSV با برچسب صحیح و چاپ/PDF از خروجی مرورگر.</p></div><div className="flex gap-2"><button disabled={!rows.length} onClick={exportCsv} className="px-3 py-2 rounded-xl bg-emerald-600 text-white text-xs font-bold disabled:opacity-40 flex items-center gap-1"><Download className="w-4 h-4" />CSV</button><button disabled={!rows.length} onClick={() => window.print()} className="px-3 py-2 rounded-xl bg-slate-800 text-white text-xs font-bold disabled:opacity-40 flex items-center gap-1"><Printer className="w-4 h-4" />چاپ / PDF</button></div></div>
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 grid md:grid-cols-2 xl:grid-cols-5 gap-3 text-xs">
      ...
      <label className="text-slate-400">Scope<select value={scopeMode} onChange={(e) => setScopeMode(e.target.value as ScopeMode)} className="field mt-1 w-full"><option value="farm">کل مجموعه</option><option value="multiHall">چند سالن</option><option value="hall">یک سالن</option><option value="pond">یک استخر</option></select></label>

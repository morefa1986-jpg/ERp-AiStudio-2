...
export const BackupRestoreView: React.FC = () => {
  ...
  const handleRestore = (e: React.FormEvent) => {
    ...
    if (res.success) {
      setStatusMessage({
        type: 'success',
        text: 'بازیابی اطلاعات با موفقیت کامل انجام شد و سیستم به حالت انتخاب شده بازگشت.',
      });
      setRestoreJson('');
    } else {
      setStatusMessage({
        type: 'error',
        text: res.message || 'خطا در بازیابی پشتیبان.',
      });
    }
  };
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-black text-white flex items-center gap-2.5">
            ...
            پشتیبان‌گیری، بازیابی اطلاعات و یکپارچگی داده‌ها (Backup & Restore)
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            استخراج کامل دیتابیس در قالب فایل رمزنگاری شده JSON با کنترل چک‌سام CRC/SHA و بازیابی ایمن
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/30 rounded-xl text-xs text-emerald-300 font-bold">
          ...
          <span>Auto Pre-Restore Snapshot Engine</span>
        </div>
      </div>

      {statusMessage && (
        <div
          className={`p-4 rounded-2xl border flex items-center gap-3 text-xs animate-fadeIn ${
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-sm space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-sm text-white">بازیابی اطلاعات (Restore JSON)</h3>
              <span className="text-[11px] text-slate-400">
                محتوای فایل پشتیبان را در کادر زیر وارد کنید
              </span>
            </div>
          </div>
          ...
          <form onSubmit={handleRestore} className="space-y-3 text-xs">
            ...
            <button
              type="submit"
              className="w-full py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-blue-600/20 transition-all"
            >
              <Upload className="w-4 h-4" />
              تأیید و بازیابی دیتابیس
            </button>
          </form>
        </div>
      </div>

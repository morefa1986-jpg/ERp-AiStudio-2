...
const EMPTY = {
  ...
  vehiclePlateNumber: '', carrierVehicleNumber: '', vehicleType: '', driverName: '', driverNationalId: '', driverPhone: '',
  ...
  const submit = (event: React.FormEvent) => {
    ...
    if (!result.success) {
      const errors: Record<string, string> = {
        GATE_PASS_REQUIRED_FIELDS: 'همه فیلدهای ضروری نگهبانی، شامل شماره ماشین حمل، راننده، صاحب کالا، حجم و کیفیت کالا باید تکمیل شوند.',
        INVALID_NATIONAL_ID: 'کد ملی راننده و صاحب کالا باید ۱۰ رقم باشد.',
        INVALID_PHONE_NUMBER: 'شماره تلفن راننده و صاحب کالا معتبر نیست.',
        ACTION_NOT_ALLOWED: 'دسترسی ثبت برگه نگهبانی برای این کاربر فعال نیست.',
      };
      setMessage(errors[result.error || ''] || result.error || 'ثبت انجام نشد');
      return;
    }
  ...
  return <div className="space-y-6 pb-12">
    ...
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
      {gatePasses.length === 0 ? <div className="text-xs text-slate-500 bg-slate-900 border border-slate-800 rounded-2xl p-6">هنوز برگه نگهبانی ثبت نشده است.</div> : gatePasses.map((pass) => <div key={pass.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
        <div className="flex justify-between border-b border-slate-800 pb-3"><div><strong className="font-mono text-amber-400">{pass.passNumber}</strong><p className="text-white text-sm mt-1">{pass.vehiclePlateNumber} · {pass.vehicleType}</p><p className="text-[10px] text-slate-500 mt-1">شماره ماشین حمل/باربری: {pass.carrierVehicleNumber}</p></div><span className="text-[10px] text-slate-400">{pass.status}</span></div>
        <div className="grid grid-cols-2 gap-2 mt-3 text-xs text-slate-300"><span>راننده: {pass.driverName}</span><span>تلفن: {pass.driverPhone}</span><span>کد ملی راننده: {pass.driverNationalId}</span><span>صاحب کالا: {pass.cargoOwnerName}</span><span>کد ملی صاحب کالا: {pass.cargoOwnerNationalId}</span><span>کالا: {pass.cargoType}</span><span>حجم: {pass.cargoVolume}</span><span>کیفیت: {pass.cargoQuality}</span><span>بارنامه: {pass.waybillNumber || '—'}</span><span>حواله: {pass.dispatchOrderNumber || '—'}</span><span>مجوز حمل: {pass.transportPermitNumber || '—'}</span><span>صدور برگه خروج: {pass.exitSheetIssuedAt ? `${pass.exitSheetIssuedAt} / ${pass.exitSheetIssuedBy || '—'}` : '—'}</span></div>
    ...
    {printPass && <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4"><div className="bg-white text-slate-950 rounded-2xl p-8 w-full max-w-3xl max-h-[90vh] overflow-auto">
      ...
      <div className="grid grid-cols-2 gap-3 mt-4 text-sm"><div>شماره برگه: <strong className="font-mono">{printPass.passNumber}</strong></div><div>وضعیت: {printPass.status}</div><div>پلاک خودرو: {printPass.vehiclePlateNumber}</div><div>شماره ماشین حمل/باربری: {printPass.carrierVehicleNumber}</div><div>نوع خودرو: {printPass.vehicleType}</div><div>راننده: {printPass.driverName}</div><div>کد ملی راننده: {printPass.driverNationalId}</div><div>تلفن راننده: {printPass.driverPhone}</div><div>صاحب کالا: {printPass.cargoOwnerName}</div><div>کد ملی صاحب کالا: {printPass.cargoOwnerNationalId}</div><div>تلفن صاحب کالا: {printPass.cargoOwnerPhone}</div><div>کالا: {printPass.cargoType}</div><div>حجم/کیفیت: {printPass.cargoVolume} · {printPass.cargoQuality}</div><div>بارنامه: {printPass.waybillNumber || '—'}</div><div>حواله: {printPass.dispatchOrderNumber || '—'}</div><div>مجوز حمل: {printPass.transportPermitNumber || '—'}</div><div>ثبت‌کننده: {printPass.registeredBy}</div><div>صادرکننده برگه خروج: {printPass.exitSheetIssuedBy || '—'}</div><div>زمان صدور: {printPass.exitSheetIssuedAt || '—'}</div><div className="col-span-2">مبدا: {printPass.originAddress}</div><div className="col-span-2">مقصد: {printPass.destinationAddress}</div></div>

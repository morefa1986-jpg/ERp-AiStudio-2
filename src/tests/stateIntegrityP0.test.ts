...
function saleState() {
  ...
  state.proformas = [{
    id: 'prof-1', stage: 'Invoice Issued (صدور فاکتور)', items: [{ id: 'item-1', sku: 'CAV-PROC-1', productName: 'Caviar', quantity: 2, unit: 'piece', unitPrice: 100, taxPercent: 0, discount: 0, total: 200 }],
  }];
  return state;
}
...
describe('P0 server invariants', () => {
  ...
  it('allows physical fulfillment only on dispatch', () => {
    const previous = saleState();
    const next = structuredClone(previous);
    next.coldStorage[0] = { ...next.coldStorage[0], unitsCount: 18, weightKg: 0.9 };
    next.proformas[0] = {

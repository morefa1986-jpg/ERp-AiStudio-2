...
function staleResult(value: number, timestamp: string | undefined, code: string, label: string): SensorValidationResult | null {
  if (!timestamp) return { isValid: false, status: 'SENSOR_FAULT', sanitizedValue: value, message: `زمان ثبت ${label} موجود نیست؛ داده قابل اعتماد نیست.`, errors: [`${code}_TIMESTAMP_MISSING`] };
  const ts = new Date(timestamp).getTime();
  if (!Number.isFinite(ts)) return { isValid: false, status: 'SENSOR_FAULT', sanitizedValue: value, message: `زمان ثبت ${label} نامعتبر است.`, errors: [`${code}_TIMESTAMP_INVALID`] };
  const ageMinutes = (Date.now() - ts) / 60_000;
  if (ageMinutes < -15) return { isValid: false, status: 'SENSOR_FAULT', sanitizedValue: value, message: `زمان ثبت ${label} در آینده است.`, errors: [`${code}_TIMESTAMP_FUTURE`] };
  if (ageMinutes > SENSOR_BOUNDS.MAX_SENSOR_AGE_MINUTES) return { isValid: false, status: 'STALE', sanitizedValue: value, message: `داده ${label} قدیمی است (${Math.round(ageMinutes)} دقیقه قبل).`, errors: [`${code}_DATA_STALE`] };
  return null;
}
...
export function validateDissolvedOxygen(rawDO: number | null | undefined, timestamp?: string): SensorValidationResult {
  ...
  if (value === null || value <= 0) return { isValid: false, status: 'SENSOR_FAULT', sanitizedValue: 0, message: 'مقدار اکسیژن محلول نامعتبر یا موجود نیست.', errors: ['DO_VALUE_INVALID'] };
  ...
  const stale = staleResult(value, timestamp, 'DO', 'اکسیژن'); if (stale) return stale;
...
export function validateWaterTemperature(rawTemp: number | null | undefined, timestamp?: string): SensorValidationResult {
  ...
  if (value === null || value < -1 || value > 40) return { isValid: false, status: 'SENSOR_FAULT', sanitizedValue: value || 0, message: 'دمای آب نامعتبر است.', errors: ['TEMP_INVALID'] };
  const stale = staleResult(value, timestamp, 'TEMP', 'دما'); if (stale) return stale;
  if (value < SENSOR_BOUNDS.TEMPERATURE.FEEDING_MIN || value > SENSOR_BOUNDS.TEMPERATURE.FEEDING_MAX) return { isValid: true, status: 'CRITICAL', sanitizedValue: value, message: `دمای ${value}°C خارج از محدوده ایمن تغذیه است.`, errors: ['TEMP_UNSAFE'] };
  return { isValid: true, status: 'VALID', sanitizedValue: value, errors: [] };
}
...
export function validatePh(rawPh: number | null | undefined, timestamp?: string): SensorValidationResult {
  ...
  if (value === null || value <= 0 || value > 14) return { isValid: false, status: 'SENSOR_FAULT', sanitizedValue: value || 0, message: 'pH نامعتبر است.', errors: ['PH_OUT_OF_PHYSICAL_RANGE'] };
  const stale = staleResult(value, timestamp, 'PH', 'pH'); if (stale) return stale;
  if (value < SENSOR_BOUNDS.PH.MIN || value > SENSOR_BOUNDS.PH.MAX) return { isValid: true, status: 'CRITICAL', sanitizedValue: value, message: `pH ${value} خارج از محدوده ایمن است.`, errors: ['PH_UNSAFE'] };
...
function validateChemical(raw: number | null | undefined, safeMax: number, criticalMax: number, code: string, label: string, timestamp?: string): SensorValidationResult {
  ...
  if (value === null || value < 0) return { isValid: false, status: 'SENSOR_FAULT', sanitizedValue: 0, message: `${label} نامعتبر یا موجود نیست.`, errors: [`${code}_INVALID`] };
  const stale = staleResult(value, timestamp, code, label); if (stale) return stale;
...
export function assessWaterSafetyForFeeding(params: PondWaterParams): WaterSafetyAssessment {
  ...
  if (params.sensorStatus === 'OFFLINE' || params.sensorStatus === 'INVALID' || params.sensorStatus === 'STALE') {
    doStatus.isValid = false; doStatus.status = params.sensorStatus === 'STALE' ? 'STALE' : params.sensorStatus === 'OFFLINE' ? 'DISCONNECTED' : 'SENSOR_FAULT'; doStatus.errors.push(`SENSOR_STATUS_${params.sensorStatus}`); doStatus.message = 'وضعیت سنسور معتبر نیست؛ تغذیه مسدود شد.';
  }
  const statuses = [doStatus, tempStatus, phStatus, ammoniaStatus, nitriteStatus];
  const invalid = statuses.find((s) => !s.isValid || ['STALE', 'SENSOR_FAULT', 'DISCONNECTED'].includes(s.status));
  const critical = statuses.find((s) => s.status === 'CRITICAL');
  return { isSafeForFeeding: !invalid && !critical, isCriticalAlert: Boolean(invalid || critical), feedingProhibitionReason: invalid?.message || critical?.message, doStatus, tempStatus, phStatus, ammoniaStatus, nitriteStatus, staleTelemetry: statuses.some((s) => s.status === 'STALE') };
}
